...
示例生产服务地址：`https://pay.example.com`

这份协议供一个业务产品的服务端使用。业务产品的网页、App 或小程序先请求自己的服务端，再由业务服务端调用中转服务；不得从浏览器直接调用，也不得把任何密钥下发给客户端。

## 1. 一次性准备

以下示例均使用 `https://pay.example.com` 作为中转服务地址；实际部署时请替换成你自己的域名。
...
若使用易支付的微信/支付宝原生收款码模式，中转服务能否收到成功结果还取决于易支付商户侧的监听设备是否在线。通常需要一台已登录收款账号的手机或挂机设备保持在线并正常监听；如果监听设备离线、被系统杀后台或网络异常，用户即使完成支付，易支付也可能无法及时上报成功，订单会继续停留在 `pending` 并在超时后变成 `expired`。
